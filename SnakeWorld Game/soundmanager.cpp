...
void			CSoundManager::UpdateAudio( float fElapsedTime )
{
    ...
    if( nFrameToApply3DAudio == 0 )
    {
        ...
        if( vListenerPos.x != listener.Position.x
			|| vListenerPos.y != listener.Position.y
			...
			locked(
				...
				for ( AudioPathList::iterator it = m_audiopaths.GetIterator();!it.EndOfList();++it )
				{
					...
					if ( pPath && pPath->pSourceVoice && pPath->bIs3D )
					{
						...
						D3DXVECTOR3 eVelocity = ( vEmiterPosScaled - pPath->emitter.Position ) / fElapsedTime;
						pPath->emitter.Position = vEmiterPosScaled;
						pPath->emitter.Velocity = eVelocity;
